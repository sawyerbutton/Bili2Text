...
主要修复：
    1. 降级处理pyannote.audio版本冲突
    2. 修复segmentation fault问题
    3. 添加更好的错误处理
    4. 优化内存管理
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('transcribe_whisperx_fixed.log', encoding='utf-8'),
...
# 转录配置
TRANSCRIBE_CONFIG = {
    "model_size": "large-v2",  # 模型大小
    "device": "cuda" if torch.cuda.is_available() else "cpu",
    "device_index": 0,
    "compute_type": "float16" if torch.cuda.is_available() else "int8",
    "batch_size": 8,  # 降低批处理大小以减少内存使用
    ...
    def transcribe_audio(self, audio_path: str) -> Dict:
        ...
        try:
            ...
            result = self.model.transcribe(
                ...
                language=language,
                task="transcribe",
                chunk_length=self.config.get("chunk_length", 30),
                print_progress=True
            )
...
def main():
    """主函数"""
    parser = argparse.ArgumentParser(description="使用WhisperX转录音频文件（修复版）")
    ...
    logger.info("="*50)
    logger.info("WhisperX音频转录工具（修复版）")
    logger.info("="*50)
